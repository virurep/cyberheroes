/* ═══════════════════════════════════════════════
   CYBERHEROES — drag & drop components
═══════════════════════════════════════════════ */

const {useState: useStateDD, useRef: useRefDD, useEffect: useEffectDD} = React;

/* ────────────────────────────────────────────────
   DragDropSort — ONE WORD AT A TIME
   Shows the active item large in the center; user drags
   it into one of the buckets, then the next item appears.
   buckets: [{ id, label, color, rgb }]
   items:   [{ id, label, bucket }]
──────────────────────────────────────────────── */
function DragDropSort({title='SORT THE WORDS', buckets, items, xp=20, onComplete}) {
  const [idx, setIdx] = useStateDD(0);
  // results: [{itemId, droppedInto, correct}]
  const [results, setResults] = useStateDD([]);
  const [hover, setHover] = useStateDD(null);
  const [dragging, setDragging] = useStateDD(false);
  const [shake, setShake] = useStateDD(false);
  const [done, setDone] = useStateDD(false);
  ...
  const handleDragEnd = () => { setDragging(false); setHover(null); };
  ...
  const handleDrop = (e, bucketId) => {
    e.preventDefault();
    setHover(null); setDragging(false);
    if (!current) return;
    ...
    setTimeout(() => {
      if (idx + 1 >= total) {
        ...
        onComplete && onComplete(allRight);
      } else {
        setIdx(i => i + 1);
      }
    }, correct ? 380 : 520);
  ...
  const reset = () => {
    setIdx(0); setResults([]); setDone(false);
    setHover(null); setDragging(false);
  };

  // tap-to-place fallback (mobile / no drag)
  const tapPlace = (bucketId) => handleDrop({preventDefault:()=>{}}, bucketId);
      ...
      <div style={{background:'var(--surf)',border:`3px solid ${allCorrect?'var(--green)':'var(--orange)'}`}}>
        <div style={{padding:'28px 22px',textAlign:'center'}}>
          <div style={{fontSize:36,marginBottom:14,
            color: allCorrect?'var(--green)':'var(--orange)'}}>
            {allCorrect ? '✓' : '⚠'}
          </div>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:8,
            marginBottom:22,maxWidth:520,margin:'0 auto 22px'}}>
            {results.map((r,i) => {
              ...
              return (
                <div key={i} style={{
                  ...
                  <span style={{color:r.correct?'var(--green)':'var(--red)',fontSize:11}}>
                    {r.correct?'✓':'✕'}
                  </span>
                  <span style={{flex:1,textAlign:'left',color:'var(--text)'}}>{item.label}</span>
                  <span style={{fontSize:7,color:'var(--dim)',letterSpacing:2}}>→</span>
                  <span style={{fontSize:7,color:correctBucket.color,letterSpacing:2}}>
                    {correctBucket.label}
                  </span>
                </div>
              );
    ...
    <div style={{background:'var(--surf)',border:'3px solid var(--green)'}}>
      ...
      <div style={{padding:'24px 22px'}}>
        ...
        <div style={{display:'flex',gap:6,justifyContent:'center',marginBottom:22}}>
          {items.map((_,i) => {
            const r = results[i];
            const bg = r ? (r.correct?'var(--green)':'var(--red)')
              : (i===idx?'var(--cyan)':'var(--border)');
            return <div key={i} style={{width:16,height:6,background:bg,
              boxShadow: i===idx ? `0 0 8px ${bg}` : 'none'}}/>;
          })}
        </div>

        {/* HUGE active word */}
        <div style={{
          display:'flex',justifyContent:'center',marginBottom:28,minHeight:120,
          alignItems:'center',
        }}>
          <div
            key={current.id}
        ...
        <div style={{fontSize:7,color:'var(--dim)',letterSpacing:3,
            ...
            return (
              <button key={b.id}
                ...
                style={{
                  ...
                  padding:'22px 14px',minHeight:90,
                  display:'flex',alignItems:'center',justifyContent:'center',
                  fontFamily:'var(--font)',
                  fontSize:'clamp(8px,1.1vw,11px)',
                  color: b.color || 'var(--cyan)',letterSpacing:2,
...
window.DragDropSort = DragDropSort;
